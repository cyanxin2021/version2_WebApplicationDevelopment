...
//	}
	public void addProduct(Order order) throws SQLException {
		...
		runner.update(DataSourceUtils.getConnection(), sql, order.getId(),
				order.getMoney(), order.getReceiverAddress(), order
						.getReceiverName(), order.getReceiverPhone(), Ordertime,
						order.getUser().getId());
	}
	/**
	 * 查找用户所属订单
	 ...
	 * @throws SQLException
	 */
	public List<Order> findOrderByManyCondition(String id, String receiverName, String userName, String category)
			throws SQLException {
		//1.创建集合对象
		List<Object> objs = new ArrayList<Object>();
		//2.定义查询sql
		String sql = "select DISTINCT orders.*,user.* from orders,user,orderitem,products where user.id=orders.user_id and orderitem.order_id=orders.id and orderitem.product_id=products.id ";
		//3.根据参数拼接sql语句
