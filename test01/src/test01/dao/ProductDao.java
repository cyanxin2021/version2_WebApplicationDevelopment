...
import test01.domain.OrderItem;
	...
	public int findAllCount(String category) throws SQLException {
		...
		if (!"全部商品".equals(category)) {
			...
			Long count = (Long) runner
					.query(sql, new ScalarHandler<Object>(), category);
...
//                             " AND orders.ordertime > DATE_SUB(NOW(), INTERVAL 7 DAY) "+
	...
	//前台，用于搜索框根据商品名和类别来模糊查询相应的商品
	public List<Product> findBookByName(int currentPage, int currentCount,
			String searchfield) throws SQLException {
		//根据名字和类别模糊查询商品
		String sql = "SELECT * FROM products WHERE name LIKE '%"+searchfield+"%' or category LIKE '%"+searchfield+"%' or description LIKE '%" +searchfield+"%'"+" LIMIT ?,?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
//		//用于分页查询的数据
//		Object obj = new Object[] { (currentPage - 1) * currentCount, currentCount };
	...
	//前台搜索框，根据商品名和类别模糊查询出的商品总数量
	public int findBookByNameAllCount(String searchfield) throws SQLException {
		String sql = "SELECT COUNT(*) FROM products WHERE name LIKE '%"+searchfield+"%' or category LIKE '%"+searchfield+"%' or description LIKE '%" +searchfield+"%'";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		//查询出满足条件的总数量，为long类型
		Long count = (Long)runner.query(sql, new ScalarHandler<Object>());
		return count.intValue();
